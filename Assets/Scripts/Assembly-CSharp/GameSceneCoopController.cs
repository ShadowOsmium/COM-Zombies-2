...
public class GameSceneCoopController : GameSceneController
{
	...
	public override void OnRewardOkButton()
	{
		...
		{
			GameData.Instance.total_cash += des_count;
	...
	private void OnObjectMessage(TNetEventData evt)
	{
		...
		if (sFSObject.ContainsKey("spawnEnemy"))
		{
			...
			EnemyType @short = (EnemyType)sFSArray.GetShort(0);
			int short2 = sFSArray.GetShort(1);
			Vector3 vector = new Vector3(sFSArray.GetFloat(2), sFSArray.GetFloat(3), sFSArray.GetFloat(4));
			EnemyFactory.CreateRemoteEnemy(@short, vector, Quaternion.identity, short2);
			Debug.Log(string.Concat("CreateRemoteEnemy:", @short, " index:", short2, " pos:", vector, " time:", Time.time));
			return;
		}
