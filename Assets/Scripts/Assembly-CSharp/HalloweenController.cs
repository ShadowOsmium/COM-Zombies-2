...
public class HalloweenController : EnemyController
{
    ...
    public void OnReplicationCast()
    {
        ...
        Debug.Log("OnReplicationCast called. This enemy's type is: " + selfController.enemy_data.enemy_type);

        var missionController = GameSceneController.Instance.mission_controller;

        var coopMission = missionController as BossCoopMissionController;
        if (coopMission != null)
        {
            coopMission.SetBoss(selfController);
            coopMission.StartCoroutine(coopMission.HalloweenSummon());
            return;
        }

        var offlineMission = missionController as BossMissionController;
        if (offlineMission != null)
        {
            offlineMission.SetBoss(selfController);
            offlineMission.StartCoroutine(offlineMission.HalloweenSummon());
            return;
        }
