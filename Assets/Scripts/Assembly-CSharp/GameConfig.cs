...
public class GameConfig : MonoBehaviour
{
	...
	public void LoadWeaponConfig()
	{
		...
		foreach (XmlElement item in documentElement.GetElementsByTagName("Weapon"))
		{
			...
			weaponConfig.price = new GameDataInt(int.Parse(item.GetAttribute("price")));
			...
			weaponConfig.bulletShopPrice = new GameDataInt(int.Parse(item.GetAttribute("bulletShopPrice")));
			weaponConfig.bulletBattletPrice = new GameDataInt(int.Parse(item.GetAttribute("bulletBalletPrice")));
			...
			weaponConfig.crystal_unlock_price = new GameDataInt(int.Parse(item.GetAttribute("CrystalUnlockPrice")));
			...
			{
				weaponConfig.unlock_day = int.Parse(item.GetAttribute("unlockDay"));
			}
	...
	public void LoadAvatarConfig()
	{
		...
		foreach (XmlElement item in documentElement.GetElementsByTagName("Avatar"))
		{
			...
			XmlElement element = item.GetElementsByTagName("HP")[0] as XmlElement;
	...
	public void LoadProbsConfig()
	{
		...
		{
			GameProbsCfg gameProbsCfg2 = new WeaponFragmentProbsCfg();
			gameProbsCfg2.prob_name = item2.GetAttribute("name");
			((WeaponFragmentProbsCfg)gameProbsCfg2).weapon_name = item2.GetAttribute("weapon");
			((WeaponFragmentProbsCfg)gameProbsCfg2).image_name = item2.GetAttribute("image");
			((WeaponFragmentProbsCfg)gameProbsCfg2).type = (WeaponFragmentProbsCfg.WeaponFragmentType)int.Parse(item2.GetAttribute("type"));
			if (item2.HasAttribute("sellPrice"))
			{
				((WeaponFragmentProbsCfg)gameProbsCfg2).sell_price = new GameDataInt(int.Parse(item2.GetAttribute("sellPrice")));
			}
	...
	public UpgradeConfig GetUpgradeCfg(XmlElement element)
	{
		...
		upgradeConfig.base_data = float.Parse(element.GetAttribute("base"));
		upgradeConfig.max_data = float.Parse(element.GetAttribute("max"));
